...
suppressMessages(library(MASS, quietly=TRUE, verbose=FALSE))
suppressMessages(library(numDeriv, quietly=TRUE, verbose=FALSE))
suppressMessages(library(plyr, quietly=TRUE, verbose=FALSE))
suppressMessages(library(fastGHQuad, quietly=TRUE, verbose=FALSE))
suppressMessages(library(batch))
suppressMessages(library(lme4))
suppressMessages(library(mvQuad))
...
mixed_logit_vem <- function(data, beta_start=rep(0, ncol(data[[1]]$x)), Sigma_start=diag(rep(1,ncol(data[[1]]$z)), ncol=ncol(data[[1]]$z)), verbose=FALSE, nodes=1) {
  P <- ncol(data[[1]]$z)
  K <- ncol(data[[1]]$x)
  n <- length(data)
  beta <- beta_start
  ...
  while(TRUE) {
    ...
    new_opt <- optim(beta,fn=elbo_psi_fixed, gr=elbo_grad_psi_fixed, Sigma=Sigma, data=data, psihats=psi_hats, nodes=nodes, verbose=verbose, control=list(fnscale=-1, trace=5), method='L-BFGS-B')
    beta <- new_opt$par
