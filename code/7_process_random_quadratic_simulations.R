...
#
# file: 7_process_random_intercept_simulations.R
#
...
save(simulation_results, file='random_quadratic_simulation_results.Rdata')
...
(g <- ggplot(subset(simulation_results, Parameter %in% c("beta[0]", "beta[1]", "beta[2]", "beta[3]", "beta[4]", "beta[5]"))) +
  ...
  xlab(NULL) +
  theme_minimal())
ggsave(filename='plots/random_quadratic_sim_estimates.png', g, width=5.5, height=3, units='in', scale=1.3)

# Compute MSEs
mses <- ddply(simulation_results, .(Parameter), function(df) {
  truth <- true_theta_df$true_theta[true_theta_df$Parameter == df$Parameter[1]]
  with(df, data.frame(lme4_MSE=mean((Estimate[Estimator == "lme4"] - truth)^2),
                      GVA_MSE=mean((Estimate[Estimator == "GVA"] - truth)^2),
                      GVAOS_MSE=mean((Estimate[Estimator == "GVA+OS"] - truth)^2)))
})

variances <- ddply(simulation_results, .(Parameter), function(df) {
  truth <- true_theta_df$true_theta[true_theta_df$Parameter == df$Parameter[1]]
  with(df, data.frame(lme4_VAR=var(Estimate[Estimator == "lme4"]),
                      GVA_VAR=var(Estimate[Estimator == "GVA"]),
                      GVAOS_VAR=var(Estimate[Estimator == "GVA+OS"])))
})
...
coverages <- ddply(simulation_results, .(Parameter, Estimator), function(df){
  if(sum(!is.na(df$CI_contains)) == 0) return(NULL)
  ptest <- prop.test(x=sum(df$CI_contains, na.rm=TRUE), n=sum(!is.na(df$CI_contains)))
...
ggplot(subset(simulation_results, Estimator=="VEM")) + 
